...
function watchTowers() {
    ...
    $['ajax']({
        async: false,
        url: 'game.php?screen=overview_villages&mode=buildings&group=0&page=-1',
        type: 'GET',
    ...
    $.each($(inc_commands), function() {
        ...
        if (found) {
            ...
            if ($(this).find('span.commandicon-wt')[0] && WT_tab[0]) {
                ...
                maxDist = 0;
                for (let i = 0; i < WT_tab.length; i++) { //sprawdzamy wszystkie wieÅ¼e
                    if (dist_strait_point(WT_tab[i], prosta, mian) < WT_tab[i].range) { // przez zasieg ktorych przechodzi atak (odleglosc od prostej ataku do wiezy < zasieg wiezy)
                        ...
                        dist > maxDist && (maxDist = dist); //wybieramy najwczeÅ›niejszy moment detekcji
                    }
                }
